...
export class Database {
  ...
  constructor(db_path = DEFAULT_DB_PATH) {
    this.db = new DatabaseSync(db_path, { enableForeignKeyConstraints: true });
    this.db.exec(SCHEMA);
  }
  ...
  get_sync_state(file_path: string): { last_modified: number; last_byte_offset: number } | undefined {
    const stmt = this.db.prepare("SELECT last_modified, last_byte_offset FROM sync_state WHERE file_path = ?");
    return stmt.get(file_path) as { last_modified: number; last_byte_offset: number } | undefined;
  }

  set_sync_state(file_path: string, last_modified: number, last_byte_offset: number) {
    const stmt = this.db.prepare(`
  ...
  get_stats() {
    ...
    const tokens = this.db.prepare(`
      ...
      FROM messages
    `).get() as { input: number; output: number; cache_read: number; cache_creation: number };

    return { sessions: sessions.count, messages: messages.count, tokens };
  }
